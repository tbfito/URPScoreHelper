 ...
 */
#ifndef lint
static const char rcsid[] = "$Id: os_win32.c,v 1.14 2002/03/18 04:40:11 shane Exp $";
#endif /* not lint */
...
int OS_SetImpersonate(int bOn)
{
	char *os_name = NULL;
	os_name = getenv("OS");
	if (bOn && os_name && stricmp(os_name, "Windows_NT") == 0) {
		bImpersonate = TRUE;
		putenv("_FCGI_NTAUTH_IMPERSONATE_=1");
		return 1;
	}
	putenv("_FCGI_NTAUTH_IMPERSONATE_=0");
	return 0;
}

void OS_GetImpersonate(void)
{
	char *impersonateEnv;
	if (bImpersonate) return;
	impersonateEnv = getenv("_FCGI_NTAUTH_IMPERSONATE_");
	if (impersonateEnv) {
		bImpersonate = atoi(impersonateEnv) != 0;
	}
}
...
int OS_SpawnChild(char *execPath, int listenFd, PROCESS_INFORMATION *pInfo, char *env)
{
    ...
    success = CreateProcess(NULL,	/* LPCSTR address of module name */
			execPath,           /* LPCSTR address of command line */
